...
class Base {
    ...
    /**
     * 基于fetch(基于Promise)的request，仅支持异步，同步请求被主流浏览器判为不友好交互
     * @param url :必填参数
    ...
    request(url, data = {}, {
        ...
        if (method === 'GET') {
            ...
            if (type === "json") {
                ...
                Object.keys(data).forEach(value => body += value + '=' + data[value] + '&');
                req.headers["Content-type"] = 'application/x-www-form-urlencoded';
                body && (req['body'] = this.trim(body, {
                    char: '&',
                    position: 'right'
                }));
            } else if (type === "none" || type === "multipart") {
            ...
            fetch(url, req)
                .then(response => {
                    ...
                    if (typeof text === 'function')
                        response.text().then(txt => text(txt));
    ...
    _drag(target, realTarget, cb) {
        ...
        window.onmousemove = e => {
            target.style.top = (tTop + e.screenY - y0) + 'px';
            target.style.left = (tLeft + e.screenX - x0) + 'px';
        };
        //卸载
        window.onmouseup = () => {
    ...
    _resize(target, realTarget, cb) {
        ...
        target.onmousemove = e => {
            ...
            if (e.layerX < padding) { //左边
                cursor = 'w-resize'
            } else if (e.layerX > tWidth - padding) {//右边
                cursor = 'e-resize';
            } else if (e.layerY < padding) {//上
                cursor = 'n-resize'
            } else if (e.layerY > tHeight - padding) {//下
                cursor = 's-resize'
            } else {
                cursor = 'default'
            }
        ...
        target.onmousedown = () => {
            ...
            window.onmousemove = moveEvent => {
                isChangeSize = true;
                diffX = moveEvent.screenX - x0;
                diffY = moveEvent.screenY - y0;
